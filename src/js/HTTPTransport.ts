import { HOST } from "../js/route-const";
...
export default class HTTPTransport {
  ...
  private request = (url: string, options: Options = {}) => {
    const { data, headers, method, timeout = 60000, withCredentials = true, responseType = "json" } = options;
    ...
    return new Promise((resolve, reject) => {
      ...
      xhr.open(method, method === HTTPMETHODS.GET && data !== undefined ? `${this.url}${url}${queryStringify(data)}` : url);
